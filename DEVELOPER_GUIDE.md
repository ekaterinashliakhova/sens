...
### Исправление проблем с карточками (отзывы и факты)

**Проблема 1:** При просмотре карточек с фактами новое сообщение перекрывало карточки.

**Решение:** 
- Добавлена функция `clearSlider()` для очистки старых карточек перед показом новых
- Класс `.slider-wrap` изменён на `.slider-wrap.current-slider` для управления жизненным циклом
- Во все функции экранов добавлен вызов `clearSlider()` перед отображением нового контента

**Проблема 2:** Отзывы появлялись в непредусмотренных местах.

**Решение:**
- Унифицирован подход к очистке слайдеров во всех функциях экранов
- Добавлены вызовы `clearSlider()` в:
  - `screen_reviews()`
  - `screen_research()`
  - `screen_faq()`
  - `screen_events()`
...
```javascript
// Добавлена новая функция для очистки слайдеров
function clearSlider() {
  document.querySelectorAll('.current-slider').forEach(el=>el.remove());
}

// Пример использования в функциях экранов
function screen_reviews(type) {
  clearChat();
  clearSlider(); // <-- Очистка старых карточек
  addBotMessage(title);
  showSlider(reviews.map(r=>({...r, isReview:true})));
}

// Обновлён класс в showSlider
function showSlider(items) {
  const wrap = document.createElement('div');
  wrap.className = 'slider-wrap current-slider'; // <-- Добавлен класс current-slider
  // ...
}
```
